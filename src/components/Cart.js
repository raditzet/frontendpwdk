...
class Cart extends Component {
    ...
    getCart = () => {
        axios.get('http://localhost:1996/carts?username='+ this.props.username)
        .then(res => {
    ...
    deleteCart = (id) => {
        axios.delete('http://localhost:1996/carts/' + id)
            .then(res => {
                this.getCart()
            })
    }
            ...
            return (
                <div className="container mt-5">
                    <div className=" row d-flex justify-content-center">
                        <h1 className="display-4">TIDAK ADA BARANG DIDALAM KERANJANG, MOHON KEMBALI KE HALAMAN <Link to='/'>ALL PRODUCTS</Link> </h1>
                    </div>
                </div>
            )
        }
